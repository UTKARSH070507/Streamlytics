...
export default function DeviceChart() {
  const { deviceData, selectedDevice, applyFilter } = useDataStore();

  const handleClick = (entry) => {
    applyFilter('device', entry.name);
  };
        ...
        <BarChart data={deviceData}>
          ...
          >
            {deviceData.map((entry, index) => (
              <Cell
                key={`cell-${index}`}
                fill={selectedDevice === entry.name ? '#ffff00' : '#10b981'}
                style={{
                  filter: selectedDevice === entry.name ? 'drop-shadow(0 0 8px #ffff00)' : 'none',
                }}
              />
            ))}
          </Bar>
